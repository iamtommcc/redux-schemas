...
describe('createAction', () => {
  it('creates a flux standard action', () => {
    expect(
      utils.createAction('ACTION', 'test payload', false, {})
    )
      .toEqual({
      type: 'ACTION',
  ...
  it('can create a standard action with partial arguments', () => {
    expect(
      utils.createAction('ACTION', 'test payload')
    )
      .toEqual({
        type: 'ACTION',
        payload: 'test payload'
...
describe('generateActionCreator', () => {
  ...
  it('generates an action creator that creates actions', () => {
    expect(utils.generateActionCreator('ACTION')('my payload', false, {}))
      .toEqual({
        type: 'ACTION',
        payload: 'my payload',
        error: false,
...
describe('generateAsyncActionCreator', () => {
  ...
  const actionToDispatch = utils.generateAsyncActionCreator('ACTION', () => axios('http://localhost/books'))();
  ...
  it('dispatches failing promises', () => {
    ...
    const expectedActions = [
      { type: 'ACTION' },
      { type: 'ACTION_FAILURE', payload: { errorMessage: 'Failed!' }, error: true }
    ];
